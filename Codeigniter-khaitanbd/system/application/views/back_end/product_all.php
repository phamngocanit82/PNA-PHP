...
function edit_check(){
	...
	<div id="div_left_margin">				
		<ul>
			...
			<li class="menu_item_click"><a href="<?=base_url()?>admin/product/all"><?=lang('product_list')?></a></li>	
...
<div id ="div_right">		
	<ul>
		<li class="li_title"><?=lang('product_list')?></li>
		<li>
			...
			<table class="table_list" border="0" cellspacing="0">
				...
				<tr>
					...
					<div style="float:left;width:400px">
						...
						<p style="margin-top:5px;margin-bottom:5px;"><b>Giá tiền : </b><?=floor($row->price)?> vnđ</p>
					</div>
					...
					<td valign="top"style="background-color:#b2d6de;border-left:solid 1px #ffffff;border-right:solid 1px #ffffff;border-bottom:solid 1px #ffffff;">
						...
						&nbsp;
						<a onclick="edit_product('<?=$row->idcategory?>','<?=$row->id?>','<?=$row->name?>','<?=$row->code?>','<?=$row->quality?>','<?=$row->description?>','<?=$row->price?>',<?=$row->lock?>);" href="<?=base_url()?>admin/category/#TB_inline?height=350&width=480&inlineId=admin_id_edit" class="thickbox" title="Chỉnh sửa sản phẩm">
