...
function CheckDelete(id){	
	...
	if (answer){
		window.location = "<?=base_url()?>admin/register02/delete/"+id;
	}
}
</script>
<div class="right">
	<div id="title"><label><?php echo(lang('admin_register_member'))?></label></div>
	<form class="register_form" id="frm_register_add" method="post" action="<?php echo(base_url())?>register02/all">
	   <?php 
		if($register02){
			for($i=0; $i<count($register02); $i++){
				$row = $register02[$i];
		?>    	    	
    	<table id="findteacher_table" >  
	    	<tr>
				<td colspan="3" align="right">				
					<a align="right" href="<?php echo(base_url())?>admin/register02/view/<?php echo($row->id)?>/<?php echo($page)?>"><?php echo(lang('continue'))?></a>
					&nbsp|&nbsp
					<?php
					if($row->lock==0){
					?>
					<a align="right" href="<?php echo(base_url())?>admin/register02/display/<?php echo($row->id)?>/<?php echo($page)?>/<?php echo($row->lock)?>"><?php echo(lang('display'))?></a>
					<?php
					}
					else{
					?>
					<a align="right" href="<?php echo(base_url())?>admin/register02/display/<?php echo($row->id)?>/<?php echo($page)?>/<?php echo($row->lock)?>"><?php echo(lang('nodisplay'))?></a>
					<?php
					}
					?>										
					&nbsp|&nbsp
