...
function CheckDelete(id){	
	...
	if (answer){
		window.location = "<?=base_url()?>admin/key_tnpt/delete/"+id;
	}
}
</script>
<div class="right">
	<div id="title"><label><?php echo(lang('admin_management_key_tnpt').' TNPT')?></label></div>
	<?php 
		if($key_tnpt){
	...
	?>
		<div style="float:left">
			<div class="newstitle">
				<a href="<?=base_url()?>admin/key_tnpt/view/<?php echo($row->id)?>"><?php echo($row->title)?></a>
				<a href="javascript:void(0)" onclick="CheckDelete(<?php echo$row->id ;?>)">
					...
					|
				<a href="<?=base_url()?>admin/key_tnpt/edit/<?php echo($row->id)?>/<?php echo($page)?>" >
