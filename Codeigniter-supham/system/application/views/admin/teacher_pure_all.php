...
function CheckDelete(id){	
	...
	if (answer){
		window.location = "<?=base_url()?>admin/teacher_pure/delete/"+id;
	}
}
</script>
<div class="right">
	<div id="title"><label><?php echo(lang('select_teacher_pure'))?></label></div>
	<form class="register_form" id="frm_teacher_pure_add" method="post" action="<?php echo(base_url())?>teacher_pure/all">
	   <?php 
		if($teacher_pure){
			for($i=0; $i<count($teacher_pure); $i++){
				$row = $teacher_pure[$i];
		?>    	    	
    	<table id="findteacher_table">  
	    	<tr>
				<td colspan="3" align="right">				
					<a align="right" href="<?php echo(base_url())?>admin/teacher_pure/select/<?php echo($row->id)?>"><?php echo(lang('select'))?></a>					
				</td>				
			</tr>
			<tr>
