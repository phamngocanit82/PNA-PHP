...
class Teacher_quality_model extends Model{
	function Teacher_quality_model(){
		parent::Model();		
	}
	function addTeacher_quality($id_user=0){
		$teacher_quality = array();
		$teacher_quality['id_user'] = intval($id_user);
		$teacher_quality['activation'] = 1; 	 
		$teacher_quality['createdate'] = date('Y-m-d H:i:s');				
		if($this->db->insert('teacher_quality', $teacher_quality)){
			return true;
		}
		return false ;
	}	
	function deleteTeacher_quality($id){
		$this->db->where('id', intval($id));		
		if ($this->db->delete('teacher_quality')){			
			return true ;
		}
		return false ;	 
	}
	...
	function getTeacher_quality($id_user){
		$query = '';
 		$query .= ' SELECT ';
 		$query .= ' 	a.id, ';
	...
	function allTeacher_quality($page = 0){
		$query = '';
 		$query .= ' SELECT g.id AS id_teacher_quality, ';
 		$query .= ' 	a.id, ';
 		$query .= ' 	a.name,a.gender, '; 
 		$query .= "     DATE_FORMAT(a.birthday, '%d') birthday_day,";
	 	$query .= "     DATE_FORMAT(a.birthday, '%m') birthday_month,";
 		...
 		$query .= ' 	a.time_teach,a.salary, ';
 		$query .= ' 	a.description,a.lock,a.typical ';
 		$query .= ' FROM teacher_quality g, user a '; 		
 		$query .= '     LEFT OUTER JOIN school b ON b.id = a.id_school ';
 		$query .= '     LEFT OUTER JOIN level c ON c.id = a.id_level_from ';
 		$query .= '     LEFT OUTER JOIN level d ON d.id = a.id_level_to '; 
	...
	function allTeacher_qualityAdmin($page = 0){
		$query = '';
 		$query .= ' SELECT ';
 		$query .= ' 	a.id, ';
 		$query .= ' 	a.name,a.gender, '; 
 		...
 		$query .= '     LEFT OUTER JOIN districts e ON e.id = a.id_districts ';
 		$query .= '     LEFT OUTER JOIN provinces f ON f.id = a.id_provinces ';	 		
		$query .= ' WHERE a.permission = 0 AND `lock`=0 AND a.id NOT IN(SELECT g.id_user FROM teacher_quality g)';	 			
 		$query .= ' ORDER BY a.editdate DESC';
 		$query .= '	LIMIT ' . $page .',' . (DEFAULT_LIMIT-15);
 		$result  = $this->db->query($query);
		$rows = $result->result() ;
