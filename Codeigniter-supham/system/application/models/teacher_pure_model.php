...
class Teacher_pure_model extends Model{
	function Teacher_pure_model(){
		parent::Model();		
	}
	function addTeacher_pure($id_user=0){
		$teacher_pure = array();
		$teacher_pure['id_user'] = intval($id_user);
		$teacher_pure['activation'] = 1; 	 
		$teacher_pure['createdate'] = date('Y-m-d H:i:s');				
		if($this->db->insert('teacher_pure', $teacher_pure)){
			return true;
		}
		return false ;
	}	
	function deleteTeacher_pure($id){
		$this->db->where('id', intval($id));		
		if ($this->db->delete('teacher_pure')){			
			return true ;
		}
		return false ;	 
	}
	...
	function getTeacher_pure($id_user){
		$query = '';
 		$query .= ' SELECT ';
 		$query .= ' 	a.id, ';
	...
	function allTeacher_pure($page = 0){
		$query = '';
 		$query .= ' SELECT g.id AS id_teacher_pure, ';
 		$query .= ' 	a.id, ';
 		$query .= ' 	a.name,a.gender, '; 
 		$query .= "     DATE_FORMAT(a.birthday, '%d') birthday_day,";
	 	$query .= "     DATE_FORMAT(a.birthday, '%m') birthday_month,";
 		...
 		$query .= ' 	a.time_teach,a.salary, ';
 		$query .= ' 	a.description,a.lock,a.typical ';
 		$query .= ' FROM teacher_pure g, user a '; 		
 		$query .= '     LEFT OUTER JOIN school b ON b.id = a.id_school ';
 		$query .= '     LEFT OUTER JOIN level c ON c.id = a.id_level_from ';
 		$query .= '     LEFT OUTER JOIN level d ON d.id = a.id_level_to '; 
	...
	function allTeacher_pureAdmin($page = 0){
		$query = '';
 		$query .= ' SELECT ';
 		$query .= ' 	a.id, ';
 		$query .= ' 	a.name,a.gender, '; 
 		...
 		$query .= '     LEFT OUTER JOIN districts e ON e.id = a.id_districts ';
 		$query .= '     LEFT OUTER JOIN provinces f ON f.id = a.id_provinces ';	 		
		$query .= ' WHERE a.permission = 0 AND `lock`=0 AND a.id NOT IN(SELECT g.id_user FROM teacher_pure g)';	 			
 		$query .= ' ORDER BY a.editdate DESC';
 		$query .= '	LIMIT ' . $page .',' . (DEFAULT_LIMIT-15);
 		$result  = $this->db->query($query);
		$rows = $result->result() ;
